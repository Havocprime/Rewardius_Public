...
async def announce_mint_to_channel(bot, user, item, method):
    """Announces a new mint to a designated Discord channel."""
    channel = bot.get_channel(MINT_CHANNEL_ID)
    if not channel:
        # Fallback: try to fetch channel if not cached
        channel = await bot.fetch_channel(MINT_CHANNEL_ID)
    rarity = item.get("rarity", "Common")
    glyph = RARITY_GLYPHS.get(rarity, "⚪️")
    float_value = item.get("float", "??.??")
    float_title = item.get("float_title", "Normal")
    roll_grade = item.get("roll_grade", "Simple")
    pitch_value = item.get("pitch_value", 0)
    embed = discord.Embed(
        title=f"✨ New Mint ({method.upper()})!",
        description=(
            f"{user.mention} minted **{item['name']}**\n\n"
            f"**Rarity:** {rarity} {glyph}\n"
            f"**Float:** `{float_value}`  [{float_title}]\n"
            f"**Roll Grade:** `{roll_grade}`\n"
            f"**Pitch Value:** `{pitch_value}pc`\n"
            f"ID: `{item.get('id', '???')}`"
        ),
        color=RARITY_COLORS.get(rarity, discord.Color.gold())
    )
    embed.set_footer(text=f"Minted by {user.display_name}")
