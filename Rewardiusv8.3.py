...
async def announce_mint_to_channel(bot, user, item, method):
    # Get the channel object (from cache or fetch)
    channel = bot.get_channel(MINTLOG_CHANNEL_ID)
    if channel is None:
        try:
            channel = await bot.fetch_channel(MINTLOG_CHANNEL_ID)
        except Exception:
            return  # Fail silently if channel is missing
    embed = discord.Embed(
        title=f"🪄 New Mint ({method.upper()})!",
        description=(
            f"**{user.mention}** minted **{item['name']}**\n"
            f"Rarity: `{item['rarity']}` | Float: `{item['float']}` [{item['float_title']}]\n"
            f"Roll Grade: `{item['roll_grade']}`\n"
            f"ID: `{item['id']}`"
        ),
        color=RARITY_COLORS.get(item.get("rarity"), discord.Color.blue())
    )
